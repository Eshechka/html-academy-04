...
function modalClickHandler(e) {
  if (e.target.classList.contains('close-button') ||
      e.target.classList.contains('modal', 'shown')) {
    modal.classList.remove('shown');
  }
}

function mainNavClickHandler(e) {
  if (!e.target.classList.contains('site-nav-link')) return;
  const clickedLink = e.target;
  const letter = clickedLink.getAttribute('href');

  const glossaryBlock = glossary.querySelector(letter);
  glossaryBlock.scrollIntoView({
    block: "start", 
    behavior: "smooth"
  });
}

function glossaryClickHandler(e) {
  clickedLink = e.target.closest('.glossary-link');
  if (!clickedLink) return;

  const term = clickedLink.textContent;
  const letter = term.slice(0, 1);

  const termData = terms.find(({key, items}) => key === letter).items
...
function render() {
    ...
    ${terms.map(({key, items}) => {
      return `
        <li class="site-nav-item">
          <a class="site-nav-link" ${items.length ? `href=#${key}` : ''}>${key.toUpperCase()}</a>
        </li>
      `
    }).join('')}
    </ul>
  `
  mainNav.insertAdjacentHTML('beforeend', mainNavMarkup);


  glossary.innerHTML = '';
  
  const glossaryMarkup = terms.map(({key, items}) => {
    return `
    <h2 id="${key}">${key.toUpperCase()}</h2>
    <ul class="glossary-list">
      ${items.map((item) => {
        return `
        <li class="glossary-item">
          <a class="glossary-link" href="#">${item.term}</a>
        </li>
        `
      }).join('')}
    </ul>
  ...
  glossary.insertAdjacentHTML('beforeend', glossaryMarkup);
...
function init() {
  ...
  glossary.addEventListener('click', glossaryClickHandler);
